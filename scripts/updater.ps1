...
if (-not $Silent) { $host.UI.RawUI.WindowTitle = 'AI Cubby Updater' }
...
$metaPath   = Join-Path (Split-Path $zipPath -Parent) 'update-meta.json'

# In silent mode: track whether we successfully launched the new exe.
# The finally block uses this to fall back to the old exe if anything went wrong.
...
try {
    ...
    # Clean up meta file so next launch doesn't re-apply the same update
    Remove-Item $metaPath -Force -EA SilentlyContinue
    ...
    # Safety net: if we're in silent mode and the new exe was never launched
    # (due to any error, crash, or early exit), always fall back to the old exe.
    if ($Silent -and -not $script:launched) {
