...
test('manual imports and opens update resource usage', () => {
  ...
  const library = read('frontend/src/pages/LibraryPage.vue')
  const card = read('frontend/src/components/ResourceCard.vue')
  assert.match(ipc, /function touchResourceUsage\(resourceId: string, learningSource\?: SearchOpenSource, explicitQuery\?: string\)/)
  assert.match(ipc, /const resource = recordProcessStart\(resourceId\)/)
  ...
  assert.match(ipc, /ipcMain\.handle\('files:openPath', async \(_e, filePath: string, meta\?: string, resourceId\?: string, searchQuery\?: string\)/)
  assert.match(ipc, /if \(resourceId\) return touchResourceUsage\(resourceId, 'app', searchQuery\)/)
  assert.match(ipc, /const resource = getResourceByPath\(filePath\)\s+return resource \? touchResourceUsage\(resource\.id, 'app', searchQuery\) : null/)
  assert.match(preload, /openPath: \(filePath: string, meta\?: string, resourceId\?: string, searchQuery\?: string\)/)
  assert.match(preload, /openAsAdmin: \(filePath: string, resourceId\?: string, searchQuery\?: string\)/)
  assert.match(apiTypes, /openPath: \(filePath: string, meta\?: string, resourceId\?: string, searchQuery\?: string\)/)
  assert.match(apiTypes, /openAsAdmin: \(filePath: string, resourceId\?: string, searchQuery\?: string\)/)
  assert.match(library, /window\.api\.files\.openPath\(resource\.file_path, resource\.meta, resource\.id, store\.searchQuery\.trim\(\) \|\| undefined\)/)
  assert.match(library, /window\.api\.files\.openAsAdmin\(listMenu\.item\.file_path, listMenu\.item\.id, store\.searchQuery\.trim\(\) \|\| undefined\)/)
  assert.match(library, /window\.api\.files\.openAsAdmin\(heatMenu\.item\.file_path, heatMenu\.item\.id, store\.searchQuery\.trim\(\) \|\| undefined\)/)
  assert.match(card, /window\.api\.files\.openAsAdmin\(props\.resource\.file_path, props\.resource\.id\)/)
  assert.match(ipc, /if \(resourceId\) return touchResourceUsage\(resourceId, 'app', searchQuery\)\s+return trackRunningProcess\(targetPath, pid\)/)
...
test('shortcut settings support direct capture and a key picker with Windows-key normalization', () => {
  const settings = read('frontend/src/pages/SettingsPage.vue')
  const main = read('backend/main.ts')
  const ipc = read('backend/ipc/index.ts')
  const zh = read('frontend/src/i18n/locales/zh.ts')

  assert.match(settings, /@keydown\.prevent="onHotkeyKeydown"/)
  assert.match(settings, /openHotkeyPicker\('wake'\)/)
  assert.match(settings, /openHotkeyPicker\('clipboard'\)/)
  assert.match(settings, /openHotkeyPicker\('pinboard'\)/)
  assert.match(settings, /if \(e\.metaKey\)\s+parts\.push\('Super'\)/)
  assert.match(settings, /\{ value: 'Super', label: 'Win' \}/)
  assert.match(settings, /hotkeyPickerError\.value = true/)
  assert.match(main, /function normalizeAccelerator\(accelerator: string\)/)
  assert.match(main, /\^\(meta\|win\|windows\)\$/i)
  assert.match(main, /new ShortcutManager\(globalShortcut, shortcutHook/)
  assert.match(main, /shortcuts\.dispose\(\)/)
  assert.match(settings, /v-model="pickerTakeover"/)
  assert.match(main, /ipcMain\.handle\('hotkey:set'/)
  assert.doesNotMatch(ipc, /ipcMain\.handle\('hotkey:set'/)
  assert.match(zh, /takeover: '尝试接管（实验性）'/)
})

test('masonry keeps layout density separate from preview resolution', () => {
  const masonry = read('frontend/src/components/MasonryWindow.vue')
  const zh = read('frontend/src/i18n/locales/zh.ts')
  const en = read('frontend/src/i18n/locales/en.ts')

  assert.match(masonry, /const colWidth = ref\(220\)/)
  assert.match(masonry, /const previewSize = ref\(512\)/)
  assert.match(masonry, /\{ size: 256, labelKey: 'masonry\.previewSmooth' \}/)
  assert.match(masonry, /\{ size: 512, labelKey: 'masonry\.previewClear' \}/)
  assert.match(masonry, /\{ size: 1024, labelKey: 'masonry\.previewHd' \}/)
  assert.match(masonry, /window\.api\.files\.readImage\(path, requestSize\)/)
  assert.match(masonry, /requestSize === previewSize\.value && requestEpoch === previewEpoch\.value/)
  assert.match(masonry, /settings\.set\('masonryPreviewSize', String\(val\)\)/)
  assert.match(masonry, /:\$\{previewEpoch\}`/)
  assert.match(masonry, /\.mw-controls \{[\s\S]*?flex-shrink: 0;/)
  assert.match(masonry, /@media \(max-width: 720px\)/)
  assert.match(zh, /previewQuality: '预览清晰度'/)
  assert.match(en, /previewQuality: 'Preview quality'/)
})
...
test('document category can create managed notes and profile documents', () => {
  ...
  const apiTypes = read('frontend/src/types/api.d.ts')
  const library = read('frontend/src/pages/LibraryPage.vue')
  const addModal = read('frontend/src/components/AddResourceModal.vue')
  ...
  assert.match(preload, /ipcRenderer\.invoke\('documents:touch', resourceId, searchQuery\)/)
  ...
  assert.match(apiTypes, /touch: \(resourceId: string, searchQuery\?: string\)/)
  assert.match(library, /showDocumentCreateEntry/)
  assert.match(library, /@document-created="onDocumentCreated"/)
  assert.match(library, /async function onDocumentCreated/)
  assert.match(addModal, /key: 'document' as const/)
  ...
  assert.match(library, /const touched = await window\.api\.documents\.touch\(resource\.id, store\.searchQuery\.trim\(\) \|\| undefined\)/)
  ...
  assert.match(library, /const isBlockElement = el\.tagName === 'DIV' \|\| el\.tagName === 'P'/)
  assert.match(library, /if \(isBlockElement\) appendTextLineBreak\(\)/)
...
test('AI recommendations stay separate from stable search results', () => {
  ...
  assert.match(library, /@click\.capture="onAiRecommendationItemClick\(\$event, item\)"/)
  assert.match(library, /if \(!suppressAiRecommendationClick\) openResource\(item\)/)
  assert.doesNotMatch(library, /startAiRecommendationsDrag[\s\S]{0,500}track\.setPointerCapture/)
  assert.match(library, /if \(!aiRecommendationDrag\.moved && Math\.abs\(distance\) > 4\)[\s\S]{0,180}track\.setPointerCapture/)
  assert.match(resourceCard, /@contextmenu\.prevent="!selectable && openMenu\(\$event\)"/)
...
test('adaptive search memory separates direct feedback from cloud-reviewed external opens', () => {
  const schema = read('backend/db/schema.ts')
  const learning = read('backend/search-learning.ts')
  const core = read('backend/search-learning-core.ts')
  const ipc = read('backend/ipc/index.ts')
  const monitor = read('backend/monitor/recent-files.ts')
  const resources = read('frontend/src/stores/resources.ts')
  const library = read('frontend/src/pages/LibraryPage.vue')
  const settings = read('frontend/src/pages/SettingsPage.vue')
  assert.match(schema, /CREATE TABLE IF NOT EXISTS search_resource_affinity/)
  assert.match(schema, /CREATE TABLE IF NOT EXISTS search_learning_judgments/)
  assert.match(schema, /FOREIGN KEY \(resource_id\) REFERENCES resources\(id\) ON DELETE CASCADE/)
  assert.match(core, /SEARCH_INTENT_TTL_MS = 30 \* 60 \* 1000/)
  assert.match(core, /SEARCH_AFFINITY_SKIP_FACTOR = 0\.82/)
  assert.match(core, /SEARCH_JUDGEMENT_MIN_CONFIDENCE = 0\.82/)
  assert.match(core, /SEARCH_JUDGEMENT_JSON_SCHEMA/)
  assert.match(learning, /recordSearchResourceOpen/)
  assert.match(learning, /enqueueJudgment/)
  assert.match(learning, /getPendingSearchJudgments/)
  assert.match(learning, /resolveSearchJudgment/)
  assert.match(learning, /if \(source === 'app'\)/)
  assert.match(ipc, /search:learned/)
  assert.match(monitor, /recordSearchResourceOpen\(fileResource\.id, 'external'\)/)
  assert.match(resources, /learnedRank = 6000/)
  assert.match(library, /window\.api\.search\.learned\(query, type\)/)
  assert.match(settings, /settings\.ai\.clearSearchLearning/)
  assert.doesNotMatch(learning, /INSERT INTO resource_tags/)
})
